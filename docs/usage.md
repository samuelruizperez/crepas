...
| `hybrid_fasta` | Path to a pre-built hybrid (endogenous + spike-in) FASTA genome file, with spike-in contigs named `<chromosome_name>_<spikein_genome>`. If provided, this is used directly instead of generating a hybrid fasta from `--fasta`/`--spikein_fasta`, and takes priority if both `--fasta` and `--hybrid_fasta` are set. | `string` |  |  |  |
...
| Parameter | Description | Type | Default | Required | Hidden |
|-----------|-----------|-----------|-----------|-----------|-----------|
| `peak_caller` | Specifies the peak calling algorithm to use. (accepted: `macs3`\|`genrich`\|`consenrich`\|`epic2`\|`mace`\|`dpeak`\|`dpos`\|`dregion`\|`dtriple`\|`edd`\|`denopa`\|`seacr`) | `string` | macs3 |  |  |
...
| `partition_plot_range` | Plot range for partition analysis visualization. | `integer` | 100 |  |  |
