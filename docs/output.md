...
| **CISRPM-SOI** | **CISRPM** **S**ignal (ChIP) **O**ver **I**nput | Keep only the bins $\alpha_i$ where $\alpha_{\text{CISRPM ChIP}i}$ and $\alpha_{\text{CISRPM input}i}$ are both > `--soi_min_count`. For the bins that pass this filter: $$\alpha_{\text{CISRPM-SOI}} = \frac{\alpha_{\text{CISRPM ChIP}}}{\alpha_{\text{CISRPM input}}}$$ | $$\alpha_{\text{CISRPM-SOI}}$$ | - |
