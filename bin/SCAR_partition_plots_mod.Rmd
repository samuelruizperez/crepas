...
partition_summary <- partition_df %>%
  dplyr::group_by(sample, sample_type) %>%
...
for (type in names(part_files)) {
  ...
  for (file in part_files[[type]]) {
    ...
    if (HAS_OKSEQ) {
      ...
      RFD_cor_mat <- partition_df %>%
        filter(RPM >= RPM_cutoff,
               !is.na(RFD_smooth)) %>%
        dplyr::select(interval, sample_type, sample, RFD_smooth)

      RFD_cor_df <- RFD_cor_mat %>%
      ...
      cor.comp <- colnames(RFD_cor_df)[2:3]
