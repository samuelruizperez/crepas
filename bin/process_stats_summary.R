...
tmp <- summary_table %>%
                ...
                mutate(
                  ...
                  ID = ifelse(grepl(paste0(".", exogenous_genome_name, "."), ID),
                              gsub("\\.flT2$", paste0(".", exogenous_genome_name, "_flT2"), ID), ID),
                  ID = ifelse(grepl(paste0(".", endogenous_genome_name, "."), ID),
                              gsub("\\.allo$", paste0(".", endogenous_genome_name, "_allo"), ID), ID),
                  ID = ifelse(grepl(paste0(".", exogenous_genome_name, "."), ID),
                              gsub("\\.allo$", paste0(".", exogenous_genome_name, "_allo"), ID), ID),
                  ID = ifelse(grepl(paste0(".", endogenous_genome_name, "."), ID),
                              gsub("\\.flT3$", paste0(".", endogenous_genome_name, "_flT3"), ID), ID),
                  ID = ifelse(grepl(paste0(".", exogenous_genome_name, "."), ID),
                              gsub("\\.flT3$", paste0(".", exogenous_genome_name, "_flT3"), ID), ID),
                  ID = ifelse(grepl(paste0(".", endogenous_genome_name, "."), ID),
                              gsub("\\..shifted$", paste0(".", endogenous_genome_name, "_shifted"), ID), ID),
                  ID = ifelse(grepl(paste0(".", exogenous_genome_name, "."), ID), 
                              gsub("\\..shifted$", paste0(".", exogenous_genome_name, "_shifted"), ID), ID),
                  ID = ifelse(grepl(paste0(".", endogenous_genome_name, "."), ID),
                              gsub("\\.dSp$", paste0(".", endogenous_genome_name, "_dSp"), ID), ID),
                  ID = ifelse(grepl(paste0(".", exogenous_genome_name, "."), ID),
                              gsub("\\.dSp$", paste0(".", exogenous_genome_name, "_dSp"), ID), ID),
