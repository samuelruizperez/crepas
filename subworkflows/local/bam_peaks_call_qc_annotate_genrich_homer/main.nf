...
workflow BAM_PEAKS_CALL_QC_ANNOTATE_GENRICH_HOMER {
    ...
    ch_bam_by_type
        ...
        .groupTuple(by: [0, 1])
        .map { id, antibody, metas, ip_bams, control_bams ->
    ...
    //
    GENRICH.out.peak.filter { meta, peaks ->
        peaks.size() > 0
    }.set { ch_gr_peaks }
    ...
    ch_bam
        ...
        .map { meta, ip_bam, control_bam, peaks ->
            [meta, ip_bam, peaks]
        }
        .transpose()
        .map { meta, ip_bam, peaks ->
