...
workflow BAM_FILTER_BLACKLIST {
    ...
    emit:
    bam      = ch_filtered_bam.se.mix(BAM_SORT_STATS_SAMTOOLS.out.bam)                    // channel: [ val(meta), [ bam ] ]
    bai      = ch_filtered_index.se.mix(BAM_SORT_STATS_SAMTOOLS.out.bai)                  // channel: [ val(meta), [ bai ] ]
