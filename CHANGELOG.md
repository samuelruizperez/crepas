...
- Support for the `Repli-seq` experiment type. An early/late (E/L) replication-timing track is calculated per sample from binned read counts across all early and late biological replicates, CPM- and quantile-normalized, combined per matched replicate pair or pooled across replicates, loess- or rolling-mean-smoothed, and written as raw and smoothed bedGraph and bigWig tracks plus a per-sample `*.qc.txt`. Adds an `rt_phase` samplesheet column (`early`, `mid` or `late`), a `test_repliseq` profile and test samplesheet, and the `repliseq_*`/`skip_repliseq_rt_tracks`/`save_repliseq_intermeds` parameters. Intermediate (`mid`) S-phase fractions take no part in the E/L track, which is computed from the early and late fractions alone. Instead, a sample with three or more fractions additionally gets a replication-timing index track, the weighted-mean fraction per bin rescaled to 0 (earliest) .. 1 (latest), which uses every fraction and so places a bin replicating in mid S-phase in the middle rather than looking like no signal at all. The MultiQC report gains a Repli-seq section summarizing, per sample, how the replicates were combined and the correlations behind that choice. `repliseq_exclude_scaffolds` (on by default) drops unplaced scaffolds and alternate sequences, which `multiBamSummary` otherwise bins along with the real chromosomes, before normalization. `repliseq_smooth_stage` chooses whether smoothing is applied to each replicate's coverage or to the ratio, `repliseq_clamp_negative` floors smoothed coverage at zero, `repliseq_ratio_direction` sets the orientation of the log2 ratio, `repliseq_loess_degree`/`repliseq_loess_family` control the fit, and `repliseq_loess_window` (bp, converted per sequence) or `repliseq_loess_span` (a fraction of bins) sets its scale. The per-sample `*.qc.txt` reports every pairwise replicate correlation, within a fraction and across fractions, regardless of which replicate-combination method was used. Alongside each ratio track a `.covered` copy is written with bins that have no reads in either fraction dropped (`repliseq_min_bin_reads`), since those still take a ratio of exactly log2(1/1) = 0 and are otherwise indistinguishable from genuinely mid-replicating bins. That covered track is then segmented into domains of constant replication timing with DNAcopy's circular binary segmentation, each domain being called early or late from its segment mean (`repliseq_domain_classification`, with an optional intermediate class), and written as one BED per class plus a combined BED carrying the segment means. `repliseq_domain_track` chooses whether to segment the raw track, the smoothed one, or both. Genes are separately classified by the S-phase fraction with the highest read density over their gene body, counted with featureCounts over the gene annotation, and written as a table of per-fraction densities plus one BED per class; genes with too few reads to call are reported as `unclassified` (`repliseq_min_gene_reads`, `repliseq_gene_min_margin`). Diagnostic plots of the class counts, the early/late ratio per class and early against late coverage are drawn with ggplot2, and the MultiQC report gains a bar chart of the class counts alongside the early/late ratio plot.
